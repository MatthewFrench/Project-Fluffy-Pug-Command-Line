...
/*
void AllyChampionManager::debugDraw(ImageData imageData) {
    for (int i = 0; i < [championBars count]; i++) {
        ChampionBar mb;
...
void AllyChampionManager::processImage(ImageData imageData) {
    double delta = (clock() - lastUpdateTime)/CLOCKS_PER_SEC;
    lastUpdateTime = clock();
    double lastFullScreenUpdate = (clock() - fullScreenUpdateTime)/CLOCKS_PER_SEC;
    if (lastFullScreenUpdate >= 0.93) { //It's been a whole second, scan the screen
        fullScreenUpdateTime = clock();
        needsFullScreenUpdate = true;
    }
...
void AllyChampionManager::scanSection(ImageData imageData, int xStart, int yStart, int xEnd, int yEnd) {
...
void AllyChampionManager::processChampionsHealth(ImageData imageData) {
    for (int i = 0; i < [championBars count]; i++) {
        ChampionBar cb;
        [[championBars objectAtIndex:i] getValue:&cb];
        cb.health = 0;
...
void AllyChampionManager::processPixel(ImageData imageData, uint8_t *pixel, int x, int y) {
    //Detect top left bar
